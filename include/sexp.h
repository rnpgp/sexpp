...
typedef struct sexp_inputstream {
  ...
  void (*getChar)(struct sexp_inputstream*);
...
typedef struct sexp_outputstream {
  ...
  void (*putChar)(struct sexp_outputstream*, int);        /* output a character */
  void (*newLine)(struct sexp_outputstream*, int);        /* go to next line (and indent) */
...
int significantNibbles(sexpOutputStream *os, sexpSimpleString *ss);
